...
class AutomationManager:
    ...
    def start_job(self, titulos, app):
        new_job = Job(total=len(titulos), status="EM_ANDAMENTO")
...
class ReenviarFalhas(Resource):
    def post(self, job_id):
        ...
        # 2. Filtra apenas as falhas TÉCNICAS
        # Ignora "E-mail não preenchido" pois isso é regra de negócio e falhará de novo
        ...
        # 3. Inicia um NOVO Job com esses títulos
...
class ListarJobs(Resource):
    def get(self):
        jobs = Job.query.order_by(Job.start_time.desc()).all()

        summary = []
        for j in jobs:
            qtd_sem_email = sum(
                1 for r in j.resultados if r.status == 'FALHA' and 'E-mail não preenchido' in (r.detalhe or ''))
            qtd_tecnico = sum(
                1 for r in j.resultados if r.status == 'FALHA' and 'E-mail não preenchido' not in (r.detalhe or ''))
